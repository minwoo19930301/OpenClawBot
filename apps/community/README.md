...
`COMMUNITY_PUSH_SUBJECT`, `COMMUNITY_PUSH_PUBLIC_KEY`, `COMMUNITY_PUSH_PRIVATE_KEY`를 모두 설정한 경우에만 알림 구독을 활성화합니다. VAPID private key와 subscription keys는 server-only 데이터이며 브라우저 응답·로그·저장소에 노출하지 않습니다. 알림 본문에는 방 메시지나 첨부 내용 대신 일반적인 새 활동 안내만 넣고, 발송 대상은 현재 방 멤버로 제한합니다. 로그아웃하면 해당 세션의 구독을 정리합니다.

PWA 설치와 Web Push 수신은 HTTPS secure context와 브라우저 권한이 필요합니다. 현재 문서는 구현 계약을 설명하며, 실제 운영 브라우저의 PWA 설치·알림 수신과 공급자별 전달 성공은 아직 검증하지 않았습니다.
...
테스트는 인증·권한·초대·첨부·데스크톱 티켓/프록시·CDP 연결·모델 도구 루프·PWA/Web Push 계약과 기본 방 동작을 점검합니다. 정확한 테스트 수는 현재 소스의 test runner 결과를 기준으로 확인하며, 공개 URL의 운영 부하와 실제 푸시 전달 성공은 별도 검증이 필요합니다.
...
- iPhone/iPad(iOS/iPadOS 16.4 이상): Safari에서 사이트를 열고 공유 → 홈 화면에 추가를 선택합니다. 홈 화면 아이콘으로 다시 열어 로그인한 뒤, 왼쪽 메뉴의 **알림 켜기**를 누르고 기기의 권한 요청을 허용합니다.
- Android: Chrome에서 사이트의 **앱으로 설치** 또는 브라우저 메뉴의 설치 항목을 사용합니다. 로그인 후 **알림 켜기**를 누릅니다.
- **테스트 알림 보내기**로 기기 수신을 확인합니다. 서버의 접수 성공과 기기의 실제 표시 여부는 다르며, 집중 모드·알림 설정·네트워크 상태가 전달에 영향을 줄 수 있습니다.
- 사람의 새 메시지는 작성자를 제외한 방 참여자에게, 봇 답변은 방 참여자에게 전달됩니다. 알림에는 대화 본문·방 이름·참여자 이름을 넣지 않습니다. 방별 알림은 같은 알림 태그로 묶으며, 이 버전의 화면에서는 기기 단위 켜기/끄기를 제공합니다.
- 오프라인 채팅이나 백그라운드에서 Linux 화면을 계속 실행하는 기능은 제공하지 않습니다. 대화·첨부·인증 응답은 서비스 워커에 캐시하지 않습니다.
