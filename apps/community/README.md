...
라이브 링크: [https://168.107.91.96](https://168.107.91.96) (운영 상태는 별도 검증이 필요합니다.)
...
설정된 방의 봇 실행에는 OpenAI 호환 Chat Completions 모델 호출과 브라우저 도구 호출이 포함될 수 있습니다. 한 턴은 최대 4개 브라우저 action과 최대 5회의 모델 호출로 제한되며, 추가 모델 호출 전에 서버 quota callback을 통과해야 합니다. 설정되지 않은 방은 브라우저 도구를 사용하지 않습니다. OpenClaw 2.0 계열 gateway는 `COMMUNITY_OPENCLAW_BASE_URL`, `COMMUNITY_OPENCLAW_TOKEN`, 선택적 agent id를 서버 환경 변수로 명시했을 때만 adapter를 통해 연결됩니다. 현재 연결 작업이 진행 중이므로 최종 운영 검증 전에는 실서비스 동작을 보장하지 않습니다.
